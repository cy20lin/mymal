    ...
    if (!listp) {
        ...
        if (MalSymbol* symp = first.get_if<MalSymbol>()) {
            if (*symp == "def!") {
                ...
                for (MalType & expr: rest) {
                    result = EVAL(expr, env);
                }
                return result;
            } else if (*symp == "if") {
                auto a = to_vector_args(rest); // args
                if (a.size() > 3 || a.size() < 2)
                    throw std::runtime_error("eval error: if: wrong if form, expected (if <cond> <true-expr> [false-expr])");
                MalType cond = EVAL(a[0], env);
                if (cond.get_if<MalNil>() || cond.get_if<MalBool>() && cond.get<MalBool>() == false) {
                    return a.size() < 3 ? MalNil() : EVAL(a[2],env);
                } else {
                    return EVAL(a[1],env);
                }
            } else if (*symp == "fn*") {
                auto a = to_array_args<2>(rest); // args
                MalList* pbinds = a[0].get_if<MalList>();
                if (!pbinds) throw std::runtime_error("eval error: fn*: expected list of symbols as bindings");
                MalType& body = a[1];
                auto clousure = [env,binds=*pbinds,body](MalType exprs)->MalType{
                    MalList* pexprs = exprs.get_if<MalList>();
                    if (!pexprs) throw std::runtime_error("eval error: fn*: expected list of values as arguments");
                    EnvPtr new_env = std::make_shared<Env>(env, binds, *pexprs);
                    return EVAL(body, new_env);
                };
                return MalFunction(clousure);
            } else {
            ...
            MalType eval_first = EVAL(first, env);
            MalType eval_rest = eval_ast(rest, env);
            MalFunction* fnp = eval_first.get_if<MalFunction>();
            if (!fnp) throw std::runtime_error("eval error: not a function object for call");
            MalFunction& fn = *fnp;
            MalType & args = eval_rest;
            return fn(args);
            // throw std::runtime_error("eval error: not implemented");
        }
    }
}
